...
@section('content')
    <div class="container px-4 py-8 mx-auto">
        <div class="flex flex-col items-center">
            <h1 class="text-4xl font-bold text-gray-800 dark:text-gray-100">Create Education Detail</h1>
            <p class="text-gray-600 dark:text-gray-400">Fill in your details</p>
        </div>
        <div class="flex flex-col items-center mt-8">
            <form action="{{ route('skill.update', $skill->id) }}" method="POST" class="w-full max-w-lg">
                @csrf
                @method('PUT')
                    ...
                    <div class="w-full px-3">
                        ...
                        <div class="flex">
                            ...
                            <input type="text" name="name" id="name"
                                ...
                                placeholder="School Name" value="{{ $skill->name }}" required>
                            @error('name')
                    ...
                    <div class="w-full px-3 mt-6">
                        ...
                        <div class="flex">
                            ...
                            <input type="text" name="rating" id="rating" min="1" max="5"
                                class="rounded-none rounded-e-lg bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full text-sm p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                placeholder="Rate your skill between 1-5" value="{{ $skill->rating }}" required>
                            @error('rating')
                                <p class="mt-2 text-xs italic text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
